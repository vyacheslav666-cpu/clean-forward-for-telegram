...
describe("outgoingMessageState", () => {
  ...
  it("never acknowledges a message Telegram rejected, and stops waiting on it", () => {
  ...
  it("reports one entry per in-flight message regardless of how many markers it carries", () => {
    appendBubble("is-outgoing is-sending", "1006.0001");
    appendBubble("", "1007");
    expect(findOutgoingInFlight(document)).toHaveLength(1);
  });

  it("excludes messages that were already in flight when the snapshot was taken", () => {
    const stuck = appendBubble("is-outgoing is-sending", "1008.0001");
    const baseline = new OutgoingInFlightBaseline(document);
    expect(baseline.findNew(document)).toEqual([]);

    const fresh = appendBubble("is-outgoing is-sending", "1009.0001");
  ...
  it("keeps excluding a stuck message after Web K rebuilds its bubble", () => {
    ...
    const stuck = appendBubble("is-outgoing is-sending", "1010.0001");
    const baseline = new OutgoingInFlightBaseline(document);
    stuck.remove();
    appendBubble("is-outgoing is-sending", "1010.0001");
