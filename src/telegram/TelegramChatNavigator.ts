...
export class TelegramChatNavigator {
  ...
  /**
   * Delays the destination proof while a send that this operation could collide with is running.
   *
   * The snapshot is taken on the first observation of the destination and is deliberately a side
   * effect of this lookup: this is the only point that knows the proven chat scope.
   */
  private findTransientBlocker(wait: NavigationWait): string | null {
    ...
    // back. It is also the batch's safety boundary: one message stuck since an offline session
    // would block restoration forever and fail the whole batch over unrelated traffic.
