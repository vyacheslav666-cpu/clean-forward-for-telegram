...
Дата проверки: 2026-08-04. Интерфейс: Telegram Web K в изолированной тестовой среде. Идентификаторы, названия чатов, сообщения, cookies и данные сессии не сохранялись.

Проверка исходного кода относится к `morethanwords/tweb` commit [`e52b5d9318848ab83316cb53138358cf49d2a27f`](https://github.com/morethanwords/tweb/commit/e52b5d9318848ab83316cb53138358cf49d2a27f). DOM и основные переходы дополнительно проверены без сохранения идентификаторов, названий чатов, сообщений или данных сессии.

## Важное замечание о безопасности исследования

В обезличенной проверке была выбрана встроенная self-chat строка, но кнопка Send и горячая клавиша подтверждения не нажимались. Оказалось, что для такого получателя и пустого текста в picker сам клик по строке является подтверждением: Telegram не создаёт промежуточный draft, а сразу вызывает `ChatInput.sendMessageWithForward(...)`. Содержимое проверки и данные аккаунта не сохранялись.
...
Переиспользовать уже запущенный штатный Forward picker через DOM небезопасно. После одиночного выбора нет надёжного промежутка, в котором выбранный peer уже доступен, а штатный callback ещё не начал forward. Для встроенного self-chat отправка начинается прямо из обработчика выбора; для нескольких получателей — из кнопки подтверждения; для одного обычного чата Telegram сначала открывает его и создаёт forward draft.
...
Top peers и основной список могут содержать один и тот же peer. Проверка подтвердила наличие дубликатов без сохранения количества строк или значений peer ID. Ссылочного `href` у строк внутри popup не было.
...
- Один встроенный self-chat, пустой footer: `openChat = false`; Telegram сразу вызывает `ChatInput.sendMessageWithForward(...)`.
...
- после клика по встроенному self-chat: вызов отправки уже начался;
...
| Один получатель | DOM ID получить легко, но встроенный self-chat может отправиться немедленно | Прямо поддерживается собственным `selectedPeer` |
...
| Риск настоящей пересылки | Высокий; подтверждён обезличенной проверкой self-chat | Низкий, если собственный picker вообще не вызывает native Forward |
...
Активного popup, attachment preview или forward draft не оставлено. Особый риск self-chat описан в начале отчёта.
